...
- 页面上下文装配已从 UI 层延迟解析改为业务入口显式装配：`GameManager` 在主场景序列化引用 `InventoryOperateManager` 与 `ShopManager`，再通过 `UIPageContextFactory` 生成 `GamingPageContext`、`ShopPageContext` 和 `PauseMenuContext`；页面本身只接受 `OpenContext` payload，缺失时直接抛出可定位异常，不再自行 `FindFirstObjectByType` 兜底，也不再保留 `ResolvingInventoryUiFacade` / `ResolvingShopUiFacade` 这类延迟解析桥接层。
- 战斗 HUD 的 Buff Tooltip 已从静态 `UITooltipPresenter.ActivePresenter` / 全局查找改为页面内显式注入：`GamingHudRegionHost` 将当前 `UITooltipPresenter` 注入 `BuffBarUI`，`BuffBarUI` 再配置新生成的 `TooltipHoverTarget`。现有 Tooltip 视觉表现继续复用 `UITooltipPresenter`，但不再阻挡 Raycast，也不再依赖全局 Presenter。
