...
- 背包页面内部不再保留只服务 `InventoryUI` 的 `IInventoryRegionView`、`InventoryRegionController`、`InventoryRegionState`、`InventoryListView`、`InventoryOperatePopupHost` 和 `InventoryUiBinder`。`InventoryUI` 已迁为 `ViewPartBase`，直接绑定显式传入或本地序列化的 `InventoryOperateManager`，维护当前选中 / 操作项状态，直接渲染背包列表并通过 Orange `UIManager.ShowPopupAsync()` 打开背包物品操作 Popup；背包列表和操作浮层仍是页面私有子 UI，不进入全局 Page Stack。
- 页面私有子视图已从 `Region` 命名和 `Assets/Scripts/UI/Regions` 目录中收口出来，并继续删除只转发一层的商店 / 战斗 / 背包包装类。当前 `GamingUIPage` 直接编排 HUD 文本、角色状态、Buff、摇杆输入和暂停按钮；`ShopUIPage` 管理真实商店 ViewPart；`GamePauseMenu` 和 `GamingUIPage` 直接调用 `InventoryUI.ConfigureSession()` / `ReleaseSession()`，不再通过 Binder 注入；`BuffBarUI` 通过 `BeginSession()` / `EndSession()` 接收 Player 与所属 `UIManager`，动态 Buff Tooltip 不再依赖全局入口。
...
- 业务子视图不再直接读取 `UIManager.Instance` 打开 Popup / Tooltip。`ViewHandle` 记录所属 `UIManager`，`ViewBase.OwnerUIManager` 由 Page 显式注入给 `InventoryUI`、`BuffBarUI` 和 `TooltipHoverTarget`，缺少装配时直接报错。设置面板、Buff、角色选择卡片 / 列表、描述器 `Describer`、背包物品、属性容器、升级卡片组、卡片动效和品质表现等挂在 Prefab 上的业务 UI 组件已迁入 `ViewPartBase`。
