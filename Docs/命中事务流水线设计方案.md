...
| 阶段 | 适合规则 | 不适合规则 |
| --- | --- | --- |
| `PreDefense` | 真实伤害标记、防御前伤害类型修正 | 暴击后增伤、吸血 |
