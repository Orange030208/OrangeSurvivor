...
public sealed class UpgradeCardWorkbenchWindow : EditorWindow
{
    ...
    private void DrawStylePane(params GUILayoutOption[] options)
    {
        ...
        if (GUILayout.Button("补齐内置稀有度"))
        {
            EnsureBuiltinPresentationProfiles();
        }
        EditorGUILayout.EndHorizontal();
        ...
        {
            EditorGUILayout.HelpBox("未选择样式目录，运行时不会应用稀有度样式。", MessageType.Warning);
        }
    ...
    private void EnsureBuiltinPresentationProfiles()
    {
        if (selectedPresentationCatalog == null)
        {
