...
public class BuffController : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        GameEventBus.Subscribe<RequestActiveBuffSnapshotEvent, int>(ownerEntity.EventBusId, OnRequestSnapshot);
        GameEventBus.Subscribe<ApplyBuffRequestedEvent, int>(ownerEntity.EventBusId, OnApplyBuffRequested);
        GameEventBus.Subscribe<RemoveBuffRequestedEvent, int>(ownerEntity.EventBusId, OnRemoveBuffRequested);
    }
    ...
    private void OnDisable()
    {
        if (ownerEntity != null)
        {
            GameEventBus.Unsubscribe<RequestActiveBuffSnapshotEvent, int>(ownerEntity.EventBusId, OnRequestSnapshot);
            GameEventBus.Unsubscribe<ApplyBuffRequestedEvent, int>(ownerEntity.EventBusId, OnApplyBuffRequested);
            GameEventBus.Unsubscribe<RemoveBuffRequestedEvent, int>(ownerEntity.EventBusId, OnRemoveBuffRequested);
        }
    ...
    private void Update()
    {
        ...
        bool changed = TickTimedBuffs(Time.deltaTime);
    ...
    private bool InstallStack(string buffId, BuffDataSO buffData, List<BuffRuntimeHandle> stacks, BuffDurationPolicy durationPolicy, float durationSeconds)
    {
        string runtimeSourceId = $"{BUFF_RUNTIME_SOURCE_PREFIX}_{buffId}_{Guid.NewGuid():N}";
        if (!FeatureInstaller.InstallSource(featureHost, runtimeSourceId, buffData))
        {
            return false;
        }
    ...
    private bool HandleOverflow(BuffDataSO buffData, List<BuffRuntimeHandle> stacks, BuffDurationPolicy durationPolicy, float durationSeconds)
    {
    ...
    private void RemoveStackAt(string buffId, List<BuffRuntimeHandle> stacks, int stackIndex, bool expired)
    {
        ...
        FeatureInstaller.RemoveSource(featureHost, handle.RuntimeSourceId);
    ...
    {
        if (ownerEntity == null || buffData == null || previousStackCount == currentStackCount)
        {
            return;
        }

        GameEventBus.Publish(new BuffStackChangedEvent(ownerEntity, buffData, previousStackCount, currentStackCount));
    }

    private void PublishStackRemovedEvent(BuffDataSO buffData, bool expired, int remainingStackCount)
    {
        if (ownerEntity == null || buffData == null)
        {
            return;
        }
        ...
        if (expired)
        {
            GameEventBus.Publish(new BuffStackExpiredEvent(ownerEntity, buffData, SINGLE_STACK_CHANGE_COUNT, remainingStackCount));
            return;
        }

        GameEventBus.Publish(new BuffStackRemovedEvent(ownerEntity, buffData, SINGLE_STACK_CHANGE_COUNT, remainingStackCount));
    }
    ...
    private static ActiveBuffSnapshot BuildSnapshot(BuffDataSO buffData, List<BuffRuntimeHandle> stacks)
    {
        ...
        return BuffRuntimeHandle.CreateMergedSnapshot(buffData, stacks.Count, buffData != null ? buffData.MaxStackCount : 0, hasDuration ? remainingDurationSeconds : 0f, hasDuration ? totalDurationSeconds : 0f);
    }

    private static void ResolveDuration(BuffApplyRequest applyRequest, BuffDataSO buffData, out BuffDurationPolicy durationPolicy, out float durationSeconds)
    {
    ...
    private void PublishSnapshot()
    {
        ...
        if (ownerEntity != null)
        {
            GameEventBus.Publish(ownerEntity.EventBusId, new ActiveBuffSnapshotChangedEvent(snapshots));
        }
    }
}
