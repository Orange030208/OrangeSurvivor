...
/// </summary>
public class WaveSpawnExecutor
{
        ...
        List<WaveSpawnRequest> requests)
    {
        ...
        {
            Debug.LogWarning($"[{nameof(WaveSpawnExecutor)}] Wave spawn pack {spawnPack.name} has no entries.");
            return;
        }
        ...
        for (int i = 0; i < entries.Count; i++)
        {
            ...
            // 包条目可独立覆盖标签；未覆盖时继承池条目的刷怪 Metadata，方便权重项和最终请求保持同一语义。
            WaveSpawnRequest request = new WaveSpawnRequest(
    ...
    private ContentRollItem? SelectEnemyContent(WaveSpawnModifierContext modifierContext)
    {
        ...
        {
            Debug.LogError($"[{nameof(WaveSpawnExecutor)}] Missing wave spawn content pool.");
            return null;
        }
    ...
    {
        if (item.TryGetMetadata(out WaveSpawnMetadata metadata))
        {
        ...
        SpawnPositionResolver spawnPositionResolver)
    {
        ...
        for (int requestIndex = 0; requestIndex < requests.Count; requestIndex++)
        {
            ...
            for (int i = 0; i < spawnRequest.SpawnCount; i++)
            {
                ...
                {
                    Debug.LogWarning($"[{nameof(WaveSpawnExecutor)}] Skipped spawning {spawnRequest.EnemyDefinition.name} because no safe spawn position could be resolved.");
                    continue;
                }
                ...
                {
                    Debug.LogError(
                        $"[{nameof(WaveSpawnExecutor)}] Failed to spawn {spawnRequest.EnemyDefinition.name} on wave {context.WaveNumber}, track '{spawnRequest.SourceTrackId}'.",
                        context.SpawnParent);
