...
public class WeaponDataSO : ItemDataSO, IInfoDocumentSource
{
    ...
    {
        InfoDocument document = new WeaponInfoBuilder().Build(WeaponInfoSource.FromData(this, level));
        string description = InfoDocumentTextFormatter.ToPlainText(document, includeHeader: false);
        return string.IsNullOrWhiteSpace(description) ? "一把可装备武器。" : description;
    }
