    ...
    where TItem : UIScrollListItemBase
{
    ...
    public void PlayReveal()
    {
        ...
        for (int i = 0; i < activeItems.Count; i++)
        {
            ...
            if (tween == null)
            {
                item.SetRevealImmediate(UIMotionAction.Show);
                visibleOrder++;
                continue;
            }
    ...
    {
        float cursor = layoutConfig.PaddingLeft;
        float centeredY = (layoutConfig.PaddingBottom - layoutConfig.PaddingTop) * 0.5f;
        ...
        for (int i = 0; i < orderedItems.Count; i++)
        {
            ...
            itemRect.anchoredPosition = new Vector2(cursor, centeredY);
    ...
    {
        float cursor = -layoutConfig.PaddingTop;
        float centeredX = (layoutConfig.PaddingLeft - layoutConfig.PaddingRight) * 0.5f;
        ...
        for (int i = 0; i < orderedItems.Count; i++)
        {
            ...
            Vector2 layoutSize = GetValidatedLayoutSize(item, i);
            itemRect.anchoredPosition = new Vector2(centeredX, cursor);
    ...
    private void UpdateContentSize()
    {
        ...
        if (layoutConfig.Direction == UIScrollListDirection.Horizontal)
        {
            float width = layoutConfig.PaddingLeft + mainAxisSize + layoutConfig.PaddingRight;
            float height = layoutConfig.PaddingTop + crossAxisMax + layoutConfig.PaddingBottom;
        ...
        float verticalWidth = layoutConfig.PaddingLeft + crossAxisMax + layoutConfig.PaddingRight;
        float verticalHeight = layoutConfig.PaddingTop + mainAxisSize + layoutConfig.PaddingBottom;
